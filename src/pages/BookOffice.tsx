...
export default function BookOffice() {
  ...
  const [isLoading, setIsLoading] = useState(false); // Add is loading state
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true); // Set loading state is true

    try {
      const response = await axios.post(
        "http://laravel11_firstofficebebwa.test/api/booking-transaction",
        {
          ...formData,
          // The rest of the field is handled on the backend
        },
        {
          headers: {
            "X-API-KEY": "dsajdkadjak12132131",
      ...
      setIsLoading(false); // Set loading state is false
    }
  };
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex items-center rounded-full border border-[#000929] px-5 gap-[10px] transition-all duration-300 focus-within:ring-2 focus-within:ring-[#0D903A] overflow-hidden">
                ...
                />
                {formErrors.find((error) => error.path.includes("date")) && (
